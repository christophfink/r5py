...
class TestTravelTimeMatrix:
    def test_travel_time_matrix_initialization(
        ...
        assert isinstance(
            travel_time_matrix.transport_network, r5py.TransportNetwork
        )
        assert isinstance(travel_time_matrix.origins, geopandas.GeoDataFrame)
        assert isinstance(
            travel_time_matrix.destinations, geopandas.GeoDataFrame
        )

        assert travel_time_matrix.origins.shape == origin_point.shape
        assert (
            travel_time_matrix.destinations.shape
    ...
    def test_snap_to_network(
        ...
        travel_times = travel_time_matrix.set_index(["from_id", "to_id"]).sort_index()
